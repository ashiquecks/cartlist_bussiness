...
Future<NetworkResponse<List<DeliverOrderModal>>> getDeliverdOrderApi({
  required BuildContext context,
  required String storeID,
}) async {
  try {
    final response = await get(Uri.parse(
        'http://18.183.210.225/cartlistapp_api/store/get_data/getdeliver_order.php?storeID=$storeID'));

    if (response.statusCode == 200) {
      var body = jsonDecode(response.body);

      List<DeliverOrderModal> dataList = [];
      body.forEach((e) {
        DeliverOrderModal orderResponse = DeliverOrderModal.fromJson(e);
