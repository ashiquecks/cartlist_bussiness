...
Future<NetworkResponse<List<ConfirmOrderModal>>> getConfirmedOrderApi({
  required BuildContext context,
  required String storeID,
}) async {
  try {
    final response = await get(Uri.parse(
        'http://18.183.210.225/cartlistapp_api/store/get_data/getconfirm_order.php?storeID=$storeID'));

    if (response.statusCode == 200) {
      var body = jsonDecode(response.body);

      List<ConfirmOrderModal> dataList = [];
      body.forEach((e) {
        ConfirmOrderModal orderResponse = ConfirmOrderModal.fromJson(e);
